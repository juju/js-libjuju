...
test("connect", (t) => {
  let ws
  const options = {
    wsclass: makeWSClass((instance) => {
      ws = instance
    }),
  }

  t.test("supports supplying callback to login", (t) => {
    connect("wss://1.2.3.4", options, (err, juju) => {
      t.equal(err, null)
      t.equal(juju instanceof Client, true)
      t.end()
    })
    ws.open()
  })
  ...
  t.test("returns promise, supports no callback", (t) => {
    const prom = connect("wss://1.2.3.4", options).then((juju) =>
      t.test("check", (t) => {
        t.equal(juju instanceof Client, true)
        t.end()
      })
  ...
  t.test("handles failure to connect via promise", (t) => {
    const reject = t.rejects(
      connect("wss://1.2.3.4", options),
      null,
      "cannot connect WebSocket: nope"
    )
    ws.close("nope")
    return reject
  })
  ...
  t.test("login failure via promise", (t) => {
    connect("wss://1.2.3.4", options).then((juju) => {
      juju
        .login({ user: "who", password: "secret" })
        .then((conn) => t.fail)
        .catch((error) => {
  ...
  t.test("connection info", (t) => {
    ...
    makeConnection(t, options, (conn, ws) => {
      ...
      t.equal(conn.info.modelTag, "model-c36a62d0-a17a-484e-87bf-a09d1b403627")
  ...
  t.test("connection info getFacade call", (t) => {
    const options = {
      facades: [
        ...
        class AllWatcherV1 extends BaseFacade {},
        class MyFacadeV1 extends BaseFacade {},
      ],
    }
    makeConnection(t, options, (conn, ws) => {
...
test("connectAndLogin", (t) => {
  ...
  t.test("connect failure", (t) => {
    const creds = {}
    connectAndLogin(url, creds, options, (err, result) => {
      t.equal(err, "cannot connect WebSocket: bad wolf")
      t.equal(result, null)
      t.end()
    })
...
test("generateModelURL", (t) => {
  t.test("returns valid url", (t) => {
    t.equal(
      generateModelURL("1.2.3.4:123", "abc-123-4"),
