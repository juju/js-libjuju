...
describe("connect", () => {
  ...
  function validateRedirectionLoginFailure(error: string) {
    requestEqual(ws.lastRequest, {
      type: "Admin",
  ...
  function validateRedirectionLoginSuccess(juju, error) {
    requestEqual(ws.lastRequest, {
      type: "Admin",
      request: "RedirectInfo",
      params: undefined,
  ...
  function validateLoginDischargeRequiredNoBakery(error) {
    requestEqual(ws.lastRequest, {
      type: "Admin",
      request: "Login",
      params: {
  ...
  function validateLoginDischargeRequiredFailure(error) {
    requestEqual(ws.lastRequest, {
      type: "Admin",
      request: "Login",
      params: { "client-version": "3.0.0", macaroons: ["fake macaroon"] },
      version: 3,
    });
  ...
  it("connection transport failure", (done) => {
    const options = {};
    makeConnection(options, (conn: Connection, ws: MockWebSocket) => {
      conn.transport.write(
        {
          type: "Test",
          request: "Test",
...
describe("connectAndLogin", () => {
  ...
  it("login redirection error failure", (done) => {
    ...
    connectAndLogin(url, creds, options)
      .then(() => fail)
      .catch((error) => {
        expect(error).toBe("cannot connect to model after redirection");
        requestEqual(ws.lastRequest, {
          type: "Admin",
          request: "RedirectInfo",
          params: undefined,
  ...
  it("login redirection error success", (done) => {
    ...
    const options = {
      ...
      onWSCreated: (instance: MockWebSocket) => {
