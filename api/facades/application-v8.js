      ...
      if (args) {
        ...
        for (let i = 0; i < args.applications.length; i++) {
          ...
          if (args.applications[i]) {
            ...
            params['applications'][i]['devices'] = {};
            args.applications[i].devices = args.applications[i].devices || {};
            for (let k in args.applications[i].devices) {
              // github.com/juju/juju/core/devices#Constraints
              if (args.applications[i].devices[k]) {
                params['applications'][i]['devices'][k] = {};
                // github.com/juju/juju/core/devices#DeviceType
                params['applications'][i]['devices'][k]['Type'] = args.applications[i].devices[k].type;
                params['applications'][i]['devices'][k]['Count'] = args.applications[i].devices[k].count;
                params['applications'][i]['devices'][k]['Attributes'] = {};
                args.applications[i].devices[k].attributes = args.applications[i].devices[k].attributes || {};
                for (let k2 in args.applications[i].devices[k].attributes) {
                  params['applications'][i]['devices'][k]['Attributes'][k2] = args.applications[i].devices[k].attributes[k2];
                }
              }
            }
  ...
  */
  scaleApplications(args, callback) {
    return new Promise((resolve, reject) => {
      // Prepare request parameters.
      let params;
      // github.com/juju/juju/apiserver/params#ScaleApplicationsParams
      if (args) {
        params = {};
        params['applications'] = [];
        args.applications = args.applications || [];
        for (let i = 0; i < args.applications.length; i++) {
          // github.com/juju/juju/apiserver/params#ScaleApplicationParams
          if (args.applications[i]) {
            params['applications'][i] = {};
            params['applications'][i]['application-tag'] = args.applications[i].applicationTag;
            params['applications'][i]['scale'] = args.applications[i].scale;
          }
        }
      }
      // Prepare the request to the Juju API.
      ...
      transform = resp => {
        let result;
        // github.com/juju/juju/apiserver/params#ScaleApplicationResults
        if (resp) {
          result = {};
          result.results = [];
          resp['results'] = resp['results'] || [];
          for (let i = 0; i < resp['results'].length; i++) {
            // github.com/juju/juju/apiserver/params#ScaleApplicationResult
            if (resp['results'][i]) {
              result.results[i] = {};
              // github.com/juju/juju/apiserver/params#Error
              if (resp['results'][i]['error']) {
              ...
              // github.com/juju/juju/apiserver/params#ScaleApplicationInfo
              if (resp['results'][i]['info']) {
                result.results[i].info = {};
                result.results[i].info.numUnits = resp['results'][i]['info']['num-units'];
              }
            }
          }
        }
...
if (wrappers.wrapApplication) {
  ...
  ApplicationV8 = wrappers.wrapApplication(ApplicationV8);
}

module.exports = ApplicationV8;