  ...
  */
  addToUnit(args, callback) {
    return new Promise((resolve, reject) => {
      ...
      transform = resp => {
        let result;
        // github.com/juju/juju/apiserver/params#AddStorageResults
        if (resp) {
          result = {};
          result.results = [];
          resp['results'] = resp['results'] || [];
          for (let i = 0; i < resp['results'].length; i++) {
            // github.com/juju/juju/apiserver/params#AddStorageResult
            if (resp['results'][i]) {
              result.results[i] = {};
              // github.com/juju/juju/apiserver/params#AddStorageDetails
              if (resp['results'][i]['result']) {
                result.results[i].result = {};
                result.results[i].result.storageTags = [];
                resp['results'][i]['result']['storage-tags'] = resp['results'][i]['result']['storage-tags'] || [];
                for (let i2 = 0; i2 < resp['results'][i]['result']['storage-tags'].length; i2++) {
                  result.results[i].result.storageTags[i2] = resp['results'][i]['result']['storage-tags'][i2];
                }
              }
  ...
  */
  import(args, callback) {
    return new Promise((resolve, reject) => {
      // Prepare request parameters.
      let params;
      // github.com/juju/juju/apiserver/params#BulkImportStorageParams
      if (args) {
        params = {};
        params['storage'] = [];
        args.storage = args.storage || [];
        for (let i = 0; i < args.storage.length; i++) {
          // github.com/juju/juju/apiserver/params#ImportStorageParams
          if (args.storage[i]) {
            params['storage'][i] = {};
            // github.com/juju/juju/apiserver/params#StorageKind
            params['storage'][i]['kind'] = args.storage[i].kind;
            params['storage'][i]['pool'] = args.storage[i].pool;
            params['storage'][i]['provider-id'] = args.storage[i].providerId;
            params['storage'][i]['storage-name'] = args.storage[i].storageName;
          }
        }
      }
      // Prepare the request to the Juju API.
      ...
      transform = resp => {
        let result;
        // github.com/juju/juju/apiserver/params#ImportStorageResults
        if (resp) {
          result = {};
          result.results = [];
          resp['results'] = resp['results'] || [];
          for (let i = 0; i < resp['results'].length; i++) {
            // github.com/juju/juju/apiserver/params#ImportStorageResult
            if (resp['results'][i]) {
              result.results[i] = {};
              // github.com/juju/juju/apiserver/params#ImportStorageDetails
              if (resp['results'][i]['result']) {
                result.results[i].result = {};
  ...
  */
  remove(args, callback) {
    return new Promise((resolve, reject) => {
      // Prepare request parameters.
      let params;
      // github.com/juju/juju/apiserver/params#RemoveStorage
      if (args) {
        params = {};
        params['storage'] = [];
        args.storage = args.storage || [];
        for (let i = 0; i < args.storage.length; i++) {
          // github.com/juju/juju/apiserver/params#RemoveStorageInstance
          if (args.storage[i]) {
            params['storage'][i] = {};
            params['storage'][i]['tag'] = args.storage[i].tag;
            params['storage'][i]['destroy-attachments'] = args.storage[i].destroyAttachments;
            params['storage'][i]['destroy-storage'] = args.storage[i].destroyStorage;
          }
        }
      }
      // Prepare the request to the Juju API.
...
if (wrappers.wrapStorage) {
  ...
  StorageV4 = wrappers.wrapStorage(StorageV4);
}

module.exports = StorageV4;