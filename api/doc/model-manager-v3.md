...
ModelManagerAPIV3 provides a way to wrap the different calls between version
  3 and version 4 of the model manager API
This API facade is available on controller connections.

To include ModelManagerV3 capabilities in your client, load it as
part of your facades, for instance:
```javascript
const {conn, logout} = await jujulib.connectAndLogin(url, credentials, {
  facades: [require('jujulib/api/facades/model-manager-v3')]
});
```
